...
class POAPService {
    ...
    /**
     * FIXED: Validate check-in time window using Local Time construction
     */
    validateCheckInTime(eventDate, startTime, endTime) {
        const now = new Date();
        ...
        // Build Start/End objects using the local date of the event
        ...
        console.log(`🕒 Validating: Now=${now.toLocaleTimeString()}, Opens=${bufferStart.toLocaleTimeString()}, Closes=${end.toLocaleTimeString()}`);

        if (now < bufferStart) {
            return {
                isValid: false,
                message: `Too early. Check-in opens at ${startTime} (30 min buffer allowed).`
            };
        }
        ...
        if (now > end) {
            return {
                isValid: false,
                message: `Check-in closed at ${endTime}.`
            };
        }
        
        return { isValid: true, message: 'Time valid' };
    }
    ...
    calculateAttendanceScore(eventDate, startTime, checkInTime) {
        ...
        if (checkInTime <= scheduledStart) return 100;
