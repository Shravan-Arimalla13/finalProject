// client/src/pages/TakeQuizPage.jsx - COMPLETE FIXED VERSION
...
const TakeQuizPage = () => {
    ...
    useEffect(() => {
        if (quiz && !result && !showExplanation && !isLoadingQuestion) {
            // FIXED: Check limit before loading
            if (answers.length < quiz.totalQuestions) {
                loadNextQuestion();
            } else {
                console.log('✅ Reached question limit, preparing to submit');
                // Auto-submit when limit reached
                submitQuiz();
            }
        }
    }, [quiz, answers, showExplanation, isLoadingQuestion]);
    ...
    const loadQuizDetails = async () => {
        try {
            ...
            if (res.data.hasPassed) {
                toast.info("You've already passed!");
                navigate(`/verify/${res.data.certificateId}`);
                return;
            }
            
            setQuiz(res.data);
            console.log(`✅ Quiz: ${res.data.topic} (${res.data.totalQuestions} questions)`);
        } catch (err) {
    ...
    const loadNextQuestion = async () => {
        // FIXED: Double-check limit
        if (answers.length >= quiz.totalQuestions) {
            console.log('⚠️ Already at limit, skipping question load');
            return;
        }

        setIsLoadingQuestion(true);
        try {
            console.log(`🔄 Loading Q${currentIndex + 1}/${quiz.totalQuestions}`);
            ...
            // FIXED: Handle limit reached response
            if (res.data.limitReached) {
                console.log('✅ Server confirmed limit reached');
                submitQuiz();
                return;
            }
            ...
            // FIXED: If we get a limit error, submit
    ...
    const handleAnswer = () => {
        ...
        console.log(`${isCorrect ? '✅' : '❌'} Q${currentIndex + 1}: ${isCorrect ? 'Correct' : 'Wrong'}`);
        
        setLastAnswerCorrect(isCorrect);
        setShowExplanation(true);
        
        // FIXED: Add answer immediately
        setAnswers(prev => {
    ...
    const submitQuiz = async () => {
        ...
        try {
            ...
            const res = await api.post('/quiz/submit', {
                quizId: quizId,
                score: correctCount // Send count, not percentage
            });
                    ...
                    <CardContent className="p-12 text-center space-y-4">
                        {submitting ? (
                            <>
                                ...
                                <h3 className="text-2xl font-bold">Analyzing Answers...</h3>
                                <p className="text-muted-foreground">Calculating your score</p>
                            </>
                        ) : (
                            <>
                                <Loader2 className="h-12 w-12 animate-spin text-indigo-600 mx-auto" />
                                <p className="text-lg">Loading question {currentIndex + 1}...</p>
                            </>
                        )}
                    </CardContent>
                </Card>
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/30 p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        key={`q${currentIndex}-${showExplanation ? 'exp' : 'q'}`}
