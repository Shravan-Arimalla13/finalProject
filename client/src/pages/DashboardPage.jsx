    ...
    <div className="group relative bg-card text-card-foreground rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border">
        ...
        <div className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
                <a href={`https://credential-api-v2.onrender.com/api/certificates/download/${cert.certificateId}`} target="_blank" rel="noopener noreferrer">
...
const SuperAdminDashboard = ({ user }) => {
    const modules = [
        { title: "Invite Faculty", desc: "Send invites to new Dept. Admins.", icon: Mail, link: "/admin/invite", color: "text-blue-500", bg: "bg-blue-500/10" },
                    ...
                    <Link to={mod.link} key={index}>
                        <Card className="h-full hover:shadow-lg transition-all cursor-pointer">
                            <CardContent className="p-6 flex flex-col items-center text-center space-y-4">
                                <div className={`h-16 w-16 rounded-full ${mod.bg} flex items-center justify-center`}>
...
const StudentDashboard = ({ user }) => {
    ...
    const connectWallet = async () => {
        ...
        try {
            ...
            const response = await api.post('/users/save-wallet', { walletAddress: account });
    ...
    if (loading) return <div className="p-8 flex justify-center"><div className="animate-spin h-8 w-8 border-4 border-blue-500 border-t-transparent rounded-full"></div></div>;
                    ...
                    <CardContent>
                        {wallet ? (
                            ...
                            <div>
                                ...
                                {walletError && <p className="text-destructive text-sm mt-2">{walletError}</p>}
                            </div>
                        )}
                    </CardContent>
                </Card>
            ...
            {latestCert && (
                <div className="space-y-4">
                    <h2 className="text-xl font-bold flex items-center">
                        <Sparkles className="h-5 w-5 mr-2 text-yellow-500" /> Newest Achievement
                    </h2>
...
function DashboardPage() {
    ...
    const renderDashboard = () => {
        ...
        switch (user.role) {
            ...
            default:
                return <Alert variant="destructive"><AlertTitle>Error</AlertTitle><AlertDescription>Unknown user role.</AlertDescription></Alert>;
        }
    };
